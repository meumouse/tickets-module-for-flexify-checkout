...
class Order {
    ...
    public function __construct() {
        ...
        // display ticket fields on customer order
        add_action( 'woocommerce_email_order_details', array( $this, 'display_fields_on_email' ), 20, 3 );
    }
    ...
    /**
     * Display ticket fields on customer emails
     *
     * @since 1.2.1
     ...
     * @return void
     */
    public function display_fields_on_email( $order, $sent_to_admin, $email_class ) {
        // Show only to customer emails (not admin)
        if ( $sent_to_admin ) {
            return;
        }
        ...
        for ( $i = 1; $i <= Helpers::ticket_count( $order ); $i++ ) {
            ...
            $cpf = get_post_meta( $order->get_id(), 'billing_cpf_' . $i, true );
            $phone = get_post_meta( $order->get_id(), 'billing_phone_' . $i, true );
            $phone_int = get_post_meta( $order->get_id(), 'billing_phone_' . $i . '_full', true );
            $email = get_post_meta( $order->get_id(), 'billing_email_' . $i, true );

            echo '<p>';
                echo '<strong>' . sprintf( __( 'Ingresso %s', 'tickets-module-for-flexify-checkout' ), $i ) . '</strong><br>';
                echo esc_html__( 'Nome: ', 'tickets-module-for-flexify-checkout' ) . esc_html( $first_name ) . ' ' . esc_html( $last_name ) . '<br>';
                echo esc_html__( 'CPF: ', 'tickets-module-for-flexify-checkout' ) . esc_html( $cpf ) . '<br>';

                if ( ! empty( $phone_int ) ) {
                    echo esc_html__( 'Telefone: ', 'tickets-module-for-flexify-checkout' ) . esc_html( $phone_int ) . '<br>';
                } else {
                    echo esc_html__( 'Telefone: ', 'tickets-module-for-flexify-checkout' ) . esc_html( $phone ) . '<br>';
                }

                echo esc_html__( 'E-mail: ', 'tickets-module-for-flexify-checkout' ) . esc_html( $email );
            echo '</p>';
        }
    }
}